...
static char kZDBannerPageViewPageIndexKey;

@interface ZDBannerView ()<UIScrollViewDelegate>
...
- (void)setupSubviews {
    ...
    self.scrollDirection = ZDBannerDirectionHorizontal;
...
- (BOOL)setScrollViewContentSizeIfNeed {
    ...
    if (count > 1) {
        ...
        contentSize = (self.scrollDirection == ZDBannerDirectionHorizontal) ? CGSizeMake(width * count, height) : CGSizeMake(width, height * count);
    }
...
- (void)setCurrentPageRate:(double)pageRate animate:(BOOL)animate scroll:(BOOL)scroll {
    ...
    // 滚动到指定分页
    if (scroll) {
        if (self.scrollDirection == ZDBannerDirectionHorizontal) { // 竖屏或者竖直
...
- (BOOL)shouldLoadPagesAtPageRate:(double)pageRate {
    ...
    ZDBannerPageView *pageView = [self.reusablePages objectForKey:@(normalIndex)];
...
- (CGRect)frameForPageViewAtIndex:(NSInteger)pageIndex {
    CGFloat width = self.frame.size.width;
    CGFloat height = self.frame.size.height;
    return (self.scrollDirection == ZDBannerDirectionHorizontal) ? CGRectMake(width * pageIndex, 0, width, height) : CGRectMake(0, height * pageIndex, width, height);
}
...
- (void)loadPageViewAtIndex:(NSInteger)pageIndex {
    ...
    ZDBannerPageView *pageView = [self.pages objectForKey:key];
    if (!pageView) {
...
- (void)handleTagGestureForPageView:(ZDBannerPageView *)pageView atPageIndex:(NSInteger)pageIndex {
    ...
    objc_setAssociatedObject(pageView.tapGesture, &kZDBannerPageViewPageIndexKey, @(pageIndex), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [pageView.tapGesture addTarget:self action:@selector(actionTap:)];
}
- (void)actionTap:(UITapGestureRecognizer *)tap {
    NSNumber *numPageIndex = objc_getAssociatedObject(tap, &kZDBannerPageViewPageIndexKey);
...
- (ZDBannerPageView *)dequeueReusablePageWithIdentifier:(NSString *)identifier {
    if (!identifier) return nil;
    NSMutableSet *pageSet = [self.reusablePages objectForKey:identifier];
    if (!pageSet || !pageSet.count) return nil;
    ZDBannerPageView *pageView = [pageSet anyObject];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.scrollDirection == ZDBannerDirectionHorizontal) { // 水平
