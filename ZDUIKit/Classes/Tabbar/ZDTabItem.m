...
+ (instancetype)buttonWithType:(UIButtonType)buttonType {
    ZDTabItem *tabItem = [super buttonWithType:buttonType];
    [tabItem setup];
    return tabItem;
}
- (void)setup {
    ...
    /// 默认数字模式  为0
    _badgeStyle = ZDTabItemBadgeStyleNumber;
    _badge = 0;
...
- (void)setBadgeStyle:(ZDTabItemBadgeStyle)badgeStyle {
 ...
 */
- (void)updateBadge {
    if (self.badgeStyle == ZDTabItemBadgeStyleNumber) {
        if (self.badge <= 0) {
    ...
    } else if (self.badgeStyle == ZDTabItemBadgeStyleDot) {
