...
CG_INLINE ZDIndicatorPosition ZDIndicatorPositionMake(CGFloat bottom, CGFloat width, CGFloat height) {
    ZDIndicatorPosition position = {bottom, width, height};
    return position;
}

typedef NS_ENUM(NSInteger, ZDTabbarIndicatorAnimationType) {
    ZDTabbarIndicatorAnimationTypeDefault = 0, // 默认
    ZDTabbarIndicatorAnimationTypeElasticity, // 弹性动画
};
...
/**
 是否能切换到指定index
 */
- (BOOL)tabbar:(ZDTabbar *)tabbar shouldSelectedItemAtIndex:(NSInteger)index;
/*
 * 将要切换到指定index
 */
- (void)tabbar:(ZDTabbar *)tabbar willSelectItemAtIndex:(NSInteger)index;
/*
 * 已经切换到指定index
 */
...
@property (nonatomic, copy) NSArray <ZDTabItem *> *items;
/// 指示器颜色
...
/// 指示器的动画类型
@property (nonatomic, assign) ZDTabbarIndicatorAnimationType indicatorAnimationStyle;
/**
