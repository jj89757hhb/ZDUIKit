...
//  ZDUIKit
//
//  Created by huhuabin on 2019/6/22.
//
...
@property (nonatomic, assign) ZDIndicatorPosition indicatorPosition;
@end
...
- (void)setup {
    ...
    _itemTitleFont = [ZDUIAssets PFMediumFont:15];
    _itemTitleSelectedFont = [ZDUIAssets PFMediumFont:17];
...
- (void)setIndicatorPosition:(ZDIndicatorPosition)indicatorPosition tapSwitchAnimated:(BOOL)animated {
    self.indicatorWidthFixTitle = NO;
    self.indicatorSwitchAnimated = animated;
...
- (void)updateItemIndicatorInsets {
    for (ZDTabItem *item in self.items) {
        if (self.indicatorWidthFixTitle) { // 指示器的宽 适应文字
            ...
            if (self.indicatorPosition.width) {
                CGRect frame = item.frameWithOutTransform;
                ZDIndicatorPosition position = self.indicatorPosition;
...
- (void)setItems:(NSArray<ZDTabItem *> *)items {
...
- (void)updateItemsFrame {
    ...
    if (self.isVertical) { // 如果竖直布局
        ...
        for (NSUInteger index = 0; index < self.items.count; index ++) {
            ZDTabItem *item = self.items[index];
...
- (void)tabItemClicked:(ZDTabItem *)item {
...
- (void)setSelectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    if (_selectedItemIndex != NSNotFound) {
        ZDTabItem *oldItem = self.items[_selectedItemIndex];
        oldItem.selected = NO;
        if (self.itemFontChangeFollowContentScroll) {
    ...
    ZDTabItem *newItem = self.items[selectedItemIndex];
...
- (void)updateSubViewsWhenParentScrollViewScroll:(UIScrollView *)scrollView {
    ...
    ZDTabItem *leftItem = self.items[leftIndex];
    ZDTabItem *rightItem = nil;
    ...
    if (self.indicatorScrollFollowContent) {
        if (self.indicatorAnimationStyle == ZDTabbarIndicatorAnimationTypeDefault) {
        ...
        } else if (self.indicatorAnimationStyle == ZDTabbarIndicatorAnimationTypeElasticity) {
            ...
            ZDTabItem *currentItem = self.items[currentIndex];
            ZDTabItem *targetItem = self.items[targetIndex];
