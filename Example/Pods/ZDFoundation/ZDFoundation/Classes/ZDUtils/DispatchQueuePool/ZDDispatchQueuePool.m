    ...
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            ...
            dispatch_once(&onceToken, ^{
                ...
                context[0] = ZDDispatchContextCreate("com.xtwx.user-interactive", count, qos);
            });
            return context[0];
        } break;
        case NSQualityOfServiceUserInitiated: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[1] = ZDDispatchContextCreate("com.xtwx.user-initiated", count, qos);
            });
            return context[1];
        } break;
        case NSQualityOfServiceUtility: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[2] = ZDDispatchContextCreate("com.xtwx.utility", count, qos);
            });
            return context[2];
        } break;
        case NSQualityOfServiceBackground: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[3] = ZDDispatchContextCreate("com.xtwx.background", count, qos);
            });
            return context[3];
        } break;
        ...
        default: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[4] = ZDDispatchContextCreate("com.xtwx.default", count, qos);
            });
            return context[4];
        } break;
    }
