...
static int _ZDKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _ZDKeyboardViewFrameObserver : NSObject
...
@implementation _ZDKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_ZDKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_ZDKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_ZDKeyboardViewFrameObserverKey);
}
...
@implementation ZDKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"ZDKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _ZDKeyboardViewFrameObserver *observer = [_ZDKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_ZDKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static ZDKeyboardManager *mgr = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (![UIApplication zd_isAppExtension]) {
...
- (UIWindow *)keyboardWindow {
    UIApplication *app = [UIApplication zd_sharedExtensionApplication];
    if (!app) return nil;
...
- (void)_notifyAllObservers {
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<ZDKeyboardObserver> observer in _observers.copy) {
