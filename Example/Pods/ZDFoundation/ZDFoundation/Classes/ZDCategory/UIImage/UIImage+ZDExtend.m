...
ZDSYNTH_DUMMY_CLASS(UIImage_ZDExtend)

@implementation UIImage (ZDExtend)

//给定imageView尺寸，截取部分图像
- (UIImage *)zd_imageForSize:(CGSize)size {
    UIImage *image = [self zd_scaleToBigOfSize:CGSizeMake(size.width*2, size.height*2)];
    return image;
}
...
{
    return [self zd_resizedImageWithName:name left:0.5 top:0.5];
}
...
- (UIImage *)zd_imageForSize:(CGSize)size vertical:(ZDImageVertical)vertical horizontal:(ZDImageHorizontal)horizontal {
    ...
    if(imageRadio>sizeRadio) {//裁剪宽
        if(horizontal == ZDImageHorizontalLeft){//左
            rect = CGRectMake(0, 0, height * sizeRadio, height);
        }else if (horizontal == ZDImageHorizontalCenter){//中
        ...
        if(horizontal == ZDImageVerticalTop){//左
            rect = CGRectMake(0, 0, width,width * size.height/size.width);
        }else if (horizontal == ZDImageVerticalCenter){//中
...
+ (UIImage *)zd_imageByScalingToMaxSize:(UIImage *)sourceImage {
    ...
    return [self zd_imageByScalingAndCroppingForSourceImage:sourceImage targetSize:targetSize];
}

+ (UIImage *)zd_imageByScalingAndCroppingForSourceImage:(UIImage *)sourceImage targetSize:(CGSize)targetSize {
